...
const returnContactObject = async (primaryContact) => {
    const { rows: secondaryContacts } = await itemsPool.query("SELECT * FROM CUSTOMER WHERE linkedId = $1", [primaryContact[0].id]);
    return {
        contact: {
...
app.post("/api/identify", async (req, res) => {
    ...
    if (!email.trim() && !phoneNumber.trim()) {
        res.json({ message: "Email or Phone Number is invalid" });
        return;
    }
    try {
        const { rows: primaryContact } = await itemsPool.query("SELECT * FROM CUSTOMER WHERE  ( email = $1 OR phoneNumber = $2) AND linkprecedence = $3  ", [email, phoneNumber, "primary"]);
        const { rows: secondaryContact } = await itemsPool.query("SELECT * FROM CUSTOMER WHERE  ( email = $1 OR phoneNumber = $2) AND linkprecedence = $3  ", [email, phoneNumber, "secondary"]);
        // check if email or phoneNumber exists
        const existingContact = await itemsPool.query("SELECT * FROM CUSTOMER WHERE email = $1 OR phoneNumber = $2", [email, phoneNumber]);
        // check if given contact already exists
        const exactContact = await itemsPool.query("SELECT * FROM CUSTOMER WHERE email = $1 AND phoneNumber = $2", [email, phoneNumber]);
        ...
        if (!isExistingInfo) {
            ...
            await itemsPool.query("INSERT INTO CUSTOMER (email, phoneNumber, linkedId, linkPrecedence) VALUES ($1, $2, $3, $4) RETURNING *", [email, phoneNumber, id, precedence]);
            const { rows: primaryContact } = await itemsPool.query("SELECT * FROM CUSTOMER WHERE  ( email = $1 OR phoneNumber = $2) AND linkprecedence = $3  ", [email, phoneNumber, "primary"]);
        ...
        if (primaryContact.length > 1) {
            ...
            if (primarySource && secondaryTarget) {
                ...
                await itemsPool.query(" UPDATE CUSTOMER SET  linkPrecedence = $1, linkedid=$2, updatedAt=NOW() WHERE id = $3 RETURNING *", ["secondary", primarySource[0].id, targetId]);
                const response = await returnContactObject(primarySource);
                res.json(response);
            }
        }
        // if primary exists, then create new secondary contact
        if (primaryContact.length == 1 && !isExactContact) {
